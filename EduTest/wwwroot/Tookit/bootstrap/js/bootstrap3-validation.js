...
!function($) {
   ...
   $.fn.validation.defaults = {
       validRules: [
           ...
           { name: 'mobilePhoneEmpt', validate: function (value) { return (!/^1[3|4|5|8]\d{9}$/.test(value) && value != ''); }, defaultMsg: '非法手机号码' },
           { name: 'mobilePhone', validate: function (value) { return (!/^1[3|4|5|8]\d{9}$/.test(value) ); }, defaultMsg: '非法手机号码' },
           ...
           { name: 'number', validate: function (value) { return (!/^(-?\d+)(\.\d+)?$/.test(value)); }, defaultMsg: '请输入数字。' },
           { name: 'xmlName', validate: function (value) { return (!/^[\u4e00-\u9fa5_a-zA-Z]+$/.test(value)); }, defaultMsg: '输入不合法。' },
        ...
        if (globalOptions.reqmark == true) {
            ...
            if(fform_style==0){
                ...
                $(obj).find('input, textarea').each(function(){
                    ...
                    controlGroup.removeClass('has-error has-success');
                    controlGroup.find("#valierr").remove();
                    valid = el.attr('requiredmessage');
                    if (valid) {
                        el.parent().after('<span id="autoreqmark" style="color:#FF9966">' + valid + '</span>')
                    };
                }); 
            };
        };//end showrequired
